...
class ConvertToRenpy:
  ...
  def __init__(self, document : Document, chunks: List[TextChunk], output_file_path : str):
    ...
    self.default_label_added = False
  ...
  def handle_styling(self, chunk: TextChunk) -> str:
    ...
    # Check if the first paragraph is a header based on its style name.
    if (chunk.paragraphs and 
        hasattr(chunk.paragraphs[0], "style") and 
        "Heading" in chunk.paragraphs[0].style.name and 
        chunk.paragraphs[0].text.strip() != ""):
      header_text = chunk.paragraphs[0].text.strip()
      header_label = header_text.replace(" ", "_").lower()
      text += "label {0}:\n\n".format(header_label)
      # Remove the header paragraph.
      chunk.paragraphs.pop(0)
    else:
      # If no header has been added yet, output a default "start" label.
      if not self.default_label_added:
        text += "label start:\n\n"

    self.default_label_added = True

    # Process all paragraphs in the chunk.
